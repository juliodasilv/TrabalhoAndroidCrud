...
public class TorcedorDAO {
    ...
    public Torcedor findById(long id) {
        String sql = "SELECT t.*, c.nome FROM " +
                TorcedorDAO.TABELA_TORCEDOR + " t INNER JOIN " +
                ClubeDAO.TABELA_CLUBES
                + " c ON t." + TorcedorDAO.COLUNA_CLUBE_ID + " = c." +
                ClubeDAO.COLUNA_ID
                + " where t." + TorcedorDAO.COLUNA_ID + " = " + id;
        SQLiteDatabase db = banco.getReadableDatabase();
    ...
    public String update(Torcedor torcedor) {
        long resultado;
        SQLiteDatabase db = banco.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(COLUNA_ID, torcedor.getId());
        values.put(COLUNA_NOME, torcedor.getNome());
        values.put(COLUNA_CLUBE_ID, torcedor.getClube().getId());
        resultado = db.update(TABELA_TORCEDOR, values, "id=?", new String[]{String.valueOf(torcedor.getId())});
        db.close();
        if (resultado == -1) {
    ...
    public void delete(int id) {
        SQLiteDatabase db = banco.getWritableDatabase();
        db.delete(TABELA_TORCEDOR, "id=?", new String[]{String.valueOf(id)});
        db.close();
    }
}
